...
#ifdef SOFTMATRIX_IMPLEMENTATION
...
sf_mat4 sf_mat_inverse(sf_mat4 const *a) {
    ...
    CORE_ASSERT(!sf_equal(det, SF_ZERO));
    ...
    for (int32_t i = 0; i < 16; ++i) res.data[i] = SF_MUL(res.data[i], rcp);
    return res;
}
...
void sf_mat_column_set(sf_vec4 const *a, int32_t column, sf_mat4 *b) {
    for (int32_t i = 0; i < 4; ++i)
        b->data[column + i * 4] = a->data[i];
}

sf_vec4 sf_mat_column_get(sf_mat4 const *a, int32_t column) {
    sf_vec4  result;
    for (int32_t i = 0; i < 4; ++i)
...
void sf_mat_row_set(sf_vec4 const *a, int32_t row, sf_mat4 *b) {
    for (int32_t i = 0; i < 4; ++i)
        b->data[(row * 4) + i] = a->data[i];
}

sf_vec4 sf_mat_row_get(sf_mat4 const *a, int32_t row) {
    sf_vec4  result;
    for (int32_t i = 0; i < 4; ++i)
...
sf_mat4 sf_mat_op_cumm(sf_mat4 const *a, sf_mat4 (*op)(sf_mat4 const*, sf_mat4 const*), int32_t count) {
    sf_mat4 res = a[0];
    for (int32_t i = 1; i < count; ++i) res = op(&res, &a[i]);
    return res;
}

sf_mat4 sf_mat_op_cumm_variadic(sf_mat4(*op)(sf_mat4 const*, sf_mat4 const*), int32_t count, ...) {
    va_list args;
    va_start(args, count);
    CORE_ASSERT(count > 1);
...
#undef SOFTMATRIX_IMPLEMENTATION
#endif