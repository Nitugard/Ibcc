...
sf sf_new_fraction(i64 a, i64 b) {
    SF_ASSERT(b != 0);
    return SF_DIV(sf_new(a), sf_new(b));
}

sf sf_from_raw(i64 a) {
    sf res;
    res.v = a;
    return res;
}
...
i32 sf_sign(sf a) {
    i32 sign = (a.v & (1ULL << 63)) >> 63;
    if(sign > 0) return -1;
    return 1;
}

i32 sf_sign_diff(sf a, sf b) {
...
sf sf_cos(sf a) {
    f64 res = cos(core_t(a));
    return super_t(res);
}

sf sf_sin(sf a) {
    f64 res = sin(core_t(a));
    return super_t(res);

}

sf sf_tan(sf a) {
    f64 res = tan(core_t(a));
    return super_t(res);
}

sf sf_asin(sf a) {
    f64 res = asin(core_t(a));
    return super_t(res);

}

sf sf_acos(sf a) {
    f64 res = acos(core_t(a));
    return super_t(res);
}

sf sf_atan(sf a) {
    f64 res = atan(core_t(a));
    return super_t(res);
}

sf sf_atan2(sf a, sf b) {
    f64 res = atan2(core_t(a), core_t(b));
    return super_t(res);
}


sf sf_array_op_cumm(sf const *a, sf (*op)(sf, sf), i32 count) {
    sf res = a[0];
    for (i32 i = 1; i < count; ++i) res = op(res, a[i]);
    return res;
}

void sf_array_op(sf (*op)(sf), i32 count, sf *array) {
    for (i32 i = 0; i < count; ++i) array[i] = op(array[i]);
}

sf sf_array_op_cumm_variadic(sf(*op)(sf, sf), i32 count, ...) {
    va_list args;
    va_start(args, count);
    SF_ASSERT(count > 1);
...
void sf_array_op_variadic(sf (*op)(sf), i32 count, ...) {
    va_list args;
    va_start(args, count);
    SF_ASSERT(count > 1);
    for (i32 i = 1; i < count; ++i) {
...
void sf_array_to_int_array(const sf *array, i32 count, i32 *out) {
    for(i32 i=0; i<count; ++i) out[i] = sf_int(array[i]);
}

void sf_array_to_float_array(const sf *array, i32 count, f32 *out) {
    for(i32 i=0; i<count; ++i) out[i] = sf_float(array[i]);
}

void sf_array_to_double_array(const sf *array, i32 count, f64 *out) {
    for(i32 i=0; i<count; ++i) out[i] = sf_double(array[i]);
}
